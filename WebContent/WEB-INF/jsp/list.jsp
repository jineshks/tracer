  	...
  	<div id="bodycontent">
		<div class="row">
			<div class="column grid-12">
				<div class="box tac">
					<ul id="tab">
						<li class="${actionBean.type eq 'task' ? 'selected' : ''}"><a href="/tracer/list/task/${actionBean.list}"><s:label for="ticket.tasks" class="bold"/></a></li>
						<li class="${actionBean.type eq 'defect' ? 'selected' : ''}"><a href="/tracer/list/defect/${actionBean.list}"><s:label for="ticket.defects" class="bold"/></a></li>
						<li class="${actionBean.type eq 'requirement' ? 'selected' : ''}"><a href="/tracer/list/requirement/${actionBean.list}"><s:label for="ticket.requirements" class="bold"/></a></li>					
						<li class="shadow"></li>
					</ul>
					
					<d:table name="${actionBean.items}" id="ticket" export="true" requestURI="/list">
					
						<d:column title="Id" property="id"></d:column>
						<d:column title="Title" media="html">
							<s:link href="/${ticket.type}/${ticket.id}"> ${ticket.title}
							</s:link>
						</d:column>
						<d:column title="Title" media="excel pdf">${ticket.title}</d:column>
						...
						<d:caption media="excel pdf">Tracer Application - Report [${actionBean.type}]</d:caption>
						<d:footer media="excel pdf">Automatic Report Generated by ${actionBean.context.loggedUser} at <%= new java.util.Date() %></d:footer>									
					</d:table>					
				</div>
			</div>
